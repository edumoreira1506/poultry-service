...
export const withPoultryParamFactory =
  (errorCallback: (res: Response, error: ApiError) => Response, { justAlive = false }: { justAlive?: boolean } = {}) =>
    (req: RequestWithPoultryAndBreeder, res: Response, next: NextFunction) => {
      return withRequestParam<Poultry>('poultryId', 'poultry', PoultryController, errorCallback)(req, res, () => {
        try {
...
export const withJustPoultryParam = withRequestParam<Poultry>('poultryId', 'poultry', PoultryController, BaseController.errorResponse)
