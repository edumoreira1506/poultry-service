export const MINIMUM_CHARACTERS_NAME = 3
export const MAXIMUM_CHARACTERS_NAME = 200

export const MINIMUM_CHARACTERS_CODE = 2
export const MAXIMUM_CHARACTERS_CODE = 6

export const MINIMUM_CHARACTERS_REGISTER = 1
export const MAXIMUM_CHARACTERS_REGISTER = 200

export const MINIMUM_CHARACTERS_DESCRIPTION = 3
export const MAXIMUM_CHARACTERS_DESCRIPTION = 250

export const AVAILABLE_PROVINCES = [
  'AC',
  'AL',
  'AP',
  'AM',
  'BA',
  'CE',
  'DF',
  'ES',
  'GO',
  'MA',
  'MT',
  'MS',
  'MG',
  'PA',
  'PB',
  'PR',
  'PE',
  'PI',
  'RJ',
  'RN',
  'RS',
  'RO',
  'RR',
  'SC',
  'SP',
  'SE',
  'TO',
]
